...
void AFountain::BeginPlay()
{
	...
	// Basic way to log
	// UE_LOG(LdwStudy, Warning, TEXT("Actor Name: %s, ID: %d, Location X : %.3f"), *GetName(), ID, GetActorLocation().X);

	// Special method to log using macro
	LSLOG_S(Warning);
	LSLOG(Warning, TEXT("Actor Name: %s, ID: %d, Location X : %.3f"), *GetName(), ID, GetActorLocation().X);
}
